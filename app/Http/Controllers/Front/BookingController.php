...
class BookingController extends Controller
{
    ...
    public function cart_delete($id)
    {
        ...
        $arr_cart_checkin = array();
        $i=0;
        foreach (session()->get('cart_checkin') as $value) {
            $arr_cart_checkin[$i] = $value;
                $i++;
            }
           $arr_cart_checkout = array();
            $i=0;
            foreach (session()->get('cart_checkout') as $value) {
                $arr_cart_checkout[$i] = $value;
                    $i++;
            }
        ...
        for ($i=0; $i<count($arr_cart_room_id); $i++)
        {
            if($arr_cart_room_id[$i] == $id)
            {
                ...
                session()->push('cart_checkin',$arr_cart_checkin[$i]);
                session()->push('cart_checkout',$arr_cart_checkout[$i]);
    ...
    public function paypal($total_price){

        $client = 'ATCqP3KUOLZBz1hBilZD7Rz_3PBq97NiTPRdLA5oN2cseQK1nxrErhaq_w2wJTSULb1iX_izkIKA30WL';
        $secret = 'ENcysOL7r5XwH9dHcOzdQYoUYev-4Wnavkb3EZpDNFr9dHpPskkb8-0wNPmIsbG8rU1pWjn3_qrWdtHq';
        $apiContext = new \PayPal\Rest\ApiContext(
            ...
            for($i=0;$i<count($arr_cart_room_id);$i++)
            {
                ...
                $d1 = explode('/',$arr_cart_checkin[$i]);
                $d2 = explode('/',$arr_cart_checkout[$i]);
                ...
                $obj->order_no = $order_no;
                $obj->checkin_date = $arr_cart_checkin[$i];
                $obj->checkout_date = $arr_cart_checkout[$i];
            ...
            for($i=0;$i<count($arr_cart_room_id);$i++) {
                ...
                $message .= '<br>Checkin Date: '.$arr_cart_checkin[$i];
                $message .= '<br>Checkout Date: '.$arr_cart_checkout[$i];
            ...
            return redirect()->back('home')->with('success','Payment is successful');
            }else{
                return redirect()->back('home')->with('error','Payment is failed');
            }
    }
    public function stripe(Request $request,$total_price){

        $stripe_secret_key = 'sk_test_51Nmvw7IVBsGr3p0vTTHuGNprAgCSh1hHS6zZ72uupk56rF1XOvXGjNVejhfBAwQxKQ8bUaXw6qPIgq7g8Wq5xT7l00Mt3apgf2';
       ...
       // $last_4 = $responseJson['payment_method_details']['card']['last4'];
        ...
        $message .= '<br>Payment Method: PayPal';
        $message .= '<br>Paid Amount: '.$total_price;
        $message .= '<br>Booking Date: '.date('d/m/Y').'<br>';
